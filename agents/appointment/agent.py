...
class AppointmentAgent:
    ...
    def run(state):
        ...
        if selected_patient:
            appointment_data["patient_id"] = selected_patient.get(
                "patient_id"
            )

        if selected_doctor:
            appointment_data["doctor_id"] = selected_doctor.get(
                "doctor_id"
            )
        ...
        # ---------------------------------------------------------
        # Handle responses to previous AppointmentAgent questions
        # ---------------------------------------------------------
        ...
        if awaiting_input == "appointment_doctor":
            ...
            if appointment_date is None:
                ...
                state["response"] = (
                    "Please enter a valid appointment date such as "
                    "today, tomorrow, Monday, or 2026-09-15."
                )
                return state
            ...
            selected_slot = parse_slot_selection(
                user_input,
                slots,
            )
            ...
            # Store the actual slot selected by the user.
            appointment_data["slot_id"] = selected_slot.get(
                "slot_id"
            )
            ...
            # Only identify the patient AFTER the slot has been selected.
            ...
            if confirmation is None:
                ...
                state["response"] = (
                    "Please reply yes to confirm the appointment "
                    "or no to cancel."
                )
                return state
            ...
            if not confirmation:
                ...
                state["response"] = (
                    "No problem. I won't book the appointment."
                )
                return state
        ...
        doctor_name = (
            request_data.get("doctor_name") or ""
        ).strip()
        ...
        if not appointment_data.get("doctor_id"):
            ...
            if not doctors:
                ...
                state["response"] = (
                    ...
                    f"'{doctor_name}'. Please enter the doctor's "
                    f"name again."
                )
                state["appointment_data"] = appointment_data
                return state
        ...
        if not appointment_data.get("appointment_date"):
            ...
            appointment_date = parse_date_from_text(
                user_input
            )
        ...
        # ---------------------------------------------------------
        # Handle time supplied directly by user
        # ---------------------------------------------------------

        if not appointment_data.get("appointment_time"):

            requested_time = parse_time_from_text(
                user_input
            )
            ...
            if requested_time:
                ...
                if selected_slot:
                    ...
                    appointment_data["appointment_time"] = (
                        format_slot_time(selected_slot)
                    )
        ...
        # ---------------------------------------------------------
        # Identify patient AFTER slot selection
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # Ask for symptoms / reason for visit
        # ---------------------------------------------------------
        ...
        if not appointment_data.get("booking_confirmed"):
            ...
            doctor_label = (
                (state.get("selected_doctor") or {})
                .get("doctor_name")
                or doctor_name
            )

            patient_label = (
                (state.get("selected_patient") or {})
                .get("first_name")
                or "the patient"
            )
        ...
        requested_slot_id = appointment_data["slot_id"]
        requested_time = appointment_data["appointment_time"]

        # IMPORTANT:
        # Verify that the exact slot selected earlier is
        # still available.
        slot_still_available = any(
            slot.get("slot_id") == requested_slot_id
        ...
        if not slot_still_available:
            ...
            if not current_slots:
                state["response"] = (
                    ...
                    "and there are no other slots on that date. "
                    "Please choose another date."
                )
            else:
                slot_list = "\n".join(
        ...
        result = AppointmentTool.create_appointment(
            {
                ...
                "appointment_time": appointment_data[
                    "appointment_time"
                ],
