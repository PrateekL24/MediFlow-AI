    ...
    def run(state):
        ...
        if awaiting_input == "appointment_doctor":
            ...
            if selected_doctor is None:
                ...
                state["response"] = "I couldn't identify the doctor. Please enter the number or name of the doctor."
                return state
            ...
            if appointment_date is None:
                ...
                state["response"] = "Please enter a valid appointment date such as today, tomorrow, Monday, or 2026-09-15."
                return state
            ...
            if selected_slot is None:
                ...
                state["response"] = "Please select an available slot by number or time, for example 2 or 10:30 AM."
                return state
            ...
            if not appointment_data.get("patient_id"):
                ...
                state["response"] = "Great. Before I book that slot, may I have the patient's mobile number?"
                return state
            ...
            if confirmation is None:
                ...
                state["response"] = "Please reply yes to confirm the appointment or no to cancel."
                return state
            if not confirmation:
        ...
        if not appointment_data.get("doctor_id"):
            ...
            if not doctors:
                state["current_agent"] = "Appointment"
                state["awaiting_input"] = "appointment_doctor"
                state["response"] = f"I couldn't find a doctor matching '{doctor_name}'. Please enter the doctor's name again."
                state["appointment_data"] = appointment_data
                return state
            if len(doctors) > 1:
                ...
                state["current_agent"] = "Appointment"
                state["awaiting_input"] = "appointment_doctor_selection"
                state["response"] = "I found multiple doctors:\n\n" + "\n".join(f"{i}. {d.get('doctor_name', '')}" for i, d in enumerate(doctors, 1)) + "\n\nWhich doctor do you mean?"
                state["appointment_data"] = appointment_data
                return state
        ...
        if not appointment_data.get("available_slots"):
            result = AppointmentTool.find_available_slots_by_doctor_and_date(appointment_data["doctor_id"], appointment_data["appointment_date"])
            slots = result.get("slots", [])
            if not slots:
                state["current_agent"] = "Appointment"
                state["awaiting_input"] = "appointment_date"
                state["response"] = "I couldn't find available slots for that date. Please choose another date."
                state["appointment_data"] = appointment_data
                return state
        ...
        if not appointment_data.get("appointment_time"):
            ...
            if requested_time:
                selected_slot = next((slot for slot in appointment_data["available_slots"] if format_slot_time(slot) == requested_time), None)
                if selected_slot:
                    appointment_data["slot_id"] = selected_slot.get("slot_id")
                    appointment_data["appointment_time"] = format_slot_time(selected_slot)
                elif intent == "book_appointment":
                    slot_list = "\n".join(f"{i}. {format_slot_time(slot)}" for i, slot in enumerate(appointment_data["available_slots"], 1))
                    state["current_agent"] = "Appointment"
                    state["awaiting_input"] = "appointment_slot_selection"
                    state["appointment_data"] = appointment_data
                    state["response"] = f"The {requested_time} slot is not available. Please choose one of these available slots:\n\n{slot_list}"
                    return state
        ...
        if not appointment_data.get("appointment_time"):
            ...
            state["appointment_data"] = appointment_data
            state["awaiting_input"] = "appointment_slot_selection"
            state["response"] = f"Available slots for {appointment_data['appointment_date']}:\n\n{slot_list}\n\nPlease select a slot by number or time if you'd like to book one."
            return state
        ...
        if not appointment_data.get("patient_id"):
            ...
            state["next_step"] = "reception"
            state["appointment_data"] = appointment_data
            state["response"] = "Before I book that slot, may I have the patient's mobile number?"
            return state
        ...
        if not appointment_data.get("symptoms"):
            ...
            state["appointment_data"] = appointment_data
            state["response"] = "Please briefly describe the symptoms or reason for the visit."
            return state

        if not appointment_data.get("booking_confirmed"):
            doctor_label = (state.get("selected_doctor") or {}).get("doctor_name") or doctor_name
            patient_label = (state.get("selected_patient") or {}).get("first_name") or "the patient"
            state["current_agent"] = "Appointment"
            state["awaiting_input"] = "booking_confirmation"
            state["appointment_data"] = appointment_data
            state["response"] = (
        ...
        if not any(format_slot_time(slot) == requested_time for slot in current_slots):
            ...
            if not current_slots:
                state["response"] = "That slot is no longer available, and there are no other slots on that date. Please choose another date."
            else:
                slot_list = "\n".join(f"{i}. {format_slot_time(slot)}" for i, slot in enumerate(current_slots, 1))
                state["response"] = "That slot is no longer available. Please choose another slot:\n\n" + slot_list
            return state
        ...
        if not result.get("success"):
            ...
            state["awaiting_input"] = "appointment_slot_selection"
            state["appointment_data"] = appointment_data
            state["response"] = result.get("message", "Appointment could not be booked. Please choose another slot.")
            return state
        ...
        state["next_step"] = None
        state["appointment_data"] = {}
        state["response"] = result.get("message", "Appointment booked successfully")
        return state
